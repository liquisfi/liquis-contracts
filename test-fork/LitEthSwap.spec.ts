...
describe("TestLitEth", () => {
    ...
    describe("join LIT:ETH 80/20 pool with LIT", () => {
        ...
        it("add LIT to pool", async () => {
            ...
            const minOut = await testEthLit.getMinOut(amount, 9980, litToken.address);
        ...
        it("fails if incorrect minout passed", async () => {
            ...
            let minOut = await testEthLit.getMinOut(amount, 10005, litToken.address);

            await expect(testEthLit.addBalToPool(amount.toString(), minOut)).to.be.revertedWith("BAL#208");

            minOut = await testEthLit.getMinOut(amount, 9980, litToken.address);
        ...
        it("fails if slippage not met (large deposit)", async () => {
            ...
            const minOut = await testEthLit.getMinOut(simpleToExactAmount(1, 24), 9980, litToken.address);
